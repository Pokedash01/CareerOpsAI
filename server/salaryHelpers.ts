...
export function extractSalaryLpa(text: string): [number, number] | undefined {
  ...
  for (const pat of SALARY_PATTERNS) {
    ...
    if (m) {
      // Check if this matched full rupee values (e.g. ₹8,00,000 - ₹14,00,000)
      if (pat === SALARY_PATTERNS[2]) {
        ...
        if (!isNaN(loRaw) && !isNaN(hiRaw)) {
          const loLpa = Math.round(loRaw / 100000);
          const hiLpa = Math.round(hiRaw / 100000);
      ...
      // Check if this matched USD ($50,000 - $80,000)
      if (pat === SALARY_PATTERNS[4]) {
        const loUsd = parseFloat(m[1].replace(/,/g, ''));
        const hiUsd = parseFloat(m[2].replace(/,/g, ''));
        if (!isNaN(loUsd) && !isNaN(hiUsd) && loUsd > 1000) {
          // Approximate conversion to Indian LPA (1 USD ≈ 85 INR -> 1000 USD = 0.85 LPA)
          const loLpa = Math.round((loUsd * 85) / 100000);
          const hiLpa = Math.round((hiUsd * 85) / 100000);
          return [Math.min(loLpa, hiLpa), Math.max(loLpa, hiLpa)];
        }
      }
...
export function extractExperienceYears(
  ...
  title?: string,
  link?: string
): { range: [number, number]; isInferred: boolean; reason?: string } | undefined {
  // Strategy 1: Check URL slug for explicit experience pattern (e.g., /4-to-9-years or /5-to-10-years on job boards)
  if (link) {
    const urlMatch =
      link.match(/(\d{1,2})[-_]to[-_](\d{1,2})[-_]years?/i) ||
      link.match(/(\d{1,2})[-_](\d{1,2})[-_]years?/i);
    if (urlMatch) {
  ...
  // Strategy 2: Multi-pattern extraction from JD text and HTML
  const combined = `${text || ''} ${rawHtml ? rawHtml.slice(0, 30000) : ''}`;
  if (combined.trim()) {
  ...
  // Strategy 3: Role Seniority Band Inference (strictly when NO experience is stated anywhere)
  if (title) {
    ...
    if (
      ...
      t.includes('sr.') ||
      t.includes('sr ') ||
      t.includes('specialist ii') ||
      t.includes('consultant ii')
    ) {
      return {
        range: [3, 6],
        isInferred: true,
    ...
    // Mid-level specialist / developer / analyst
    return {
      range: [2, 4],
      isInferred: true,
