...
export interface EmailDispatchResult {
  success: boolean;
  provider: 'smtp' | 'resend' | 'sendgrid' | 'ethereal' | 'fallback_log';
  messageId?: string;
  previewUrl?: string;
  error?: string;
}
...
/**
 * Creates or retrieves a nodemailer transporter based on environment configuration.
 */
...
/**
 * Formats a clean, high-production HTML email for CareerOps AI Password Reset.
 */
function buildPasswordResetHtml(name: string, email: string, code: string, minutes: number): string {
...
<body>
  <div class="card">
    ...
    <div class="footer">
      ...
      © ${new Date().getFullYear()} CareerOps AI. All rights reserved.
    </div>
  </div>
</body>
</html>
...
/**
 * Dispatches a password reset email via configured SMTP, Resend, Sendgrid, or Ethereal/Local logger.
 */
export async function sendPasswordResetEmail(options: SendResetEmailOptions): Promise<EmailDispatchResult> {
  ...
  const subject = `🔐 ${code} is your CareerOps AI password reset verification code`;
  const htmlContent = buildPasswordResetHtml(recipientName, cleanEmail, code, expiresInMinutes);
  const textContent =
    `CareerOps AI Password Reset Request\n\n` +
  ...
  // 4. Ethereal Test Account Fallback (Nodemailer free test mailbox)
  try {
  ...
  // 5. Final fallback: safe server log
  console.log(`[Mailer:Local] 🔐 Password reset verification code for ${cleanEmail} is: ${code} (expires in ${expiresInMinutes}m)`);
  return {
    success: true,
